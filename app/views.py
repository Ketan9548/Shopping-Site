...
def remove_cart(request):
    if request.method == 'GET':
        ...
        try:
            # Use first() instead of get() to handle the case when multiple objects are returned
            ...
            # Handle the case when multiple carts are found
            data = {'error': 'Multiple carts found. Unable to remove.'}
            return JsonResponse(data, status=400)
